class PostureDetector {
  constructor() {
    ...
    this.postureSuggestedShown = false;
    this.badPostureStartTime = null;

    this.postureData = JSON.parse(localStorage.getItem("postureData")) || [];
  ...
  async startCamera() {
    try {
      ...
      alert(
        "Error accessing camera. Please make sure you have a camera connected and have granted permission."
      );
    }
  }
  ...
  updateStats(neckAngle, torsoAngle, shoulderOffset) {
    document.getElementById("neckAngle").textContent = `${Math.round(
      neckAngle
    )}°`;
    document.getElementById("torsoAngle").textContent = `${Math.round(
      torsoAngle
    )}°`;
    document.getElementById("shoulderOffset").textContent = `${Math.round(
      shoulderOffset
    )}px`;
    ...
    if (neckAngle < 40 && torsoAngle < 10) {
      ...
      document.getElementById("postureTimer").textContent = `${(
        this.goodFrames * deltaTime
      ).toFixed(1)}s (Good)`;
      document.getElementById("postureTimer").className = "good";
      this.badPostureStartTime = null;
      this.showWarning("GOOD POSTURE. KEEP WORKING!", "good");
    } else {
      this.badFrames++;
      this.goodFrames = 0;
      document.getElementById("postureTimer").textContent = `${(
        this.badFrames * deltaTime
      ).toFixed(1)}s (Bad)`;
      document.getElementById("postureTimer").className = "bad";

      if (neckAngle >= 40) {
        suggestionText =
          "Sit straight, keep your head facing towards the camera.";
      } else if (torsoAngle >= 10) {
        suggestionText = "Keep your torso upright.";
      }

      if (!this.badPostureStartTime) {
        this.badPostureStartTime = Date.now();
      }
      if (Date.now() - this.badPostureStartTime > 3000 && suggestionText) {
        this.showWarning(suggestionText, "bad");
      }
    }
  }
  ...
  showWarning(message) {
    ...
    if (this.postureSuggestedShown) return;
    this.postureSuggestedShown = true;
  ...
  drawResults(results) {
    ...
    // Log data to localStorage
    const dataPoint = {
      timestamp: new Date().toISOString(),
      neck_inclination: neckInclination,
      torso_inclination: torsoInclination,
      shoulder_alignment: shoulderOffset,
      posture_status:
        neckInclination < 40 && torsoInclination < 10 ? "good" : "bad",
    };
    this.postureData.push(dataPoint);
    localStorage.setItem("postureData", JSON.stringify(this.postureData));
    ...
    const color =
      neckInclination < 40 && torsoInclination < 10 ? "#90EE90" : "#FF0000";
  ...
  generateChart() {
    ...
    // Group data by date
    const dailyData = {};
    this.postureData.forEach((point) => {
      ...
      if (point.posture_status === "good") {
        dailyData[date].good++;
      } else {
        dailyData[date].bad++;
      }
      dailyData[date].neckAngles.push(point.neck_inclination);
      dailyData[date].torsoAngles.push(point.torso_inclination);
    });

    const dates = Object.keys(dailyData);
    const goodPercentages = dates.map(
      (date) => (dailyData[date].good / dailyData[date].total) * 100
    );
    const badPercentages = dates.map(
      (date) => (dailyData[date].bad / dailyData[date].total) * 100
    );
    const avgNeckAngles = dates.map(
      (date) =>
        dailyData[date].neckAngles.reduce((a, b) => a + b, 0) /
        dailyData[date].neckAngles.length
    );
    const avgTorsoAngles = dates.map(
      (date) =>
        dailyData[date].torsoAngles.reduce((a, b) => a + b, 0) /
        dailyData[date].torsoAngles.length
    );
...
window.addEventListener("load", () => {
  if (window.Pose && window.Camera) {
    ...
    alert(
      "Error: Required libraries not loaded. Please check your internet connection and try again."
    );
  }
});
